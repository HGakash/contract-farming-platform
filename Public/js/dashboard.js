    ...
    try {
        ...
        if (contractsResponse.ok) {
            ...
            if (contractsResult.length === 0) {
                contractsList.innerHTML = '<p>No accepted contracts found.</p>';
            } else {
                contractsList.innerHTML = contractsResult.map(contract => `
                    <div class="p-4 mb-4 bg-gray-200 rounded shadow">
                        <h3 class="text-lg font-bold">${contract.companyName}</h3>
                        <p><strong>Details:</strong> ${contract.contractDetails}</p>
